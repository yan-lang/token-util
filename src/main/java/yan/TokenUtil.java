...
public class TokenUtil implements Callable<Integer> {
    ...
    @Option(names = {"-o", "--output"}, defaultValue = "Tokens.java", description = "Output file for result")
    File outputFile;
    ...
    private String generateCode(List<TokenType> tokenTypes) {
        ...
        for (var tokenType : tokenTypes) {
            builder.append(String.format("    int %s = %d;\n", tokenType.name, tokenType.id, tokenType.description))
                    .append('\n');
            descriptions.add(String.format("\"%s\"", tokenType.description));
        }
        builder.append(String.format("    List<String> tokenNames = List.of(%s);", String.join(", ", descriptions)));
        builder.append("\n}");
    ...
    private Integer getID() {
        while (usedIDs.contains(availableID)) availableID += 1;
